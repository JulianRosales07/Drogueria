...
export type ParsedProductRow = {
  ...
  minStock: number
  isActive: boolean
  presentations: Array<{ name: string; factor: number; price: number; barcode: string }>
}
...
export function downloadInventoryTemplate() {
  ...
  const instructions = [
    ...
    { Campo: 'Activo', Obligatorio: 'No', Descripcion: 'Si / No. Default Si. Los inactivos no aparecen en el POS.' },
    {
      ...
      Obligatorio: 'No',
      Descripcion:
        'Hasta 3 presentaciones adicionales además de la Unidad (ej. Caja x10, Media caja, Caja completa). Unidades debe ser mayor a 1. Deja las 4 columnas vacías si no aplica.',
    },
  ]
...
export async function parseInventoryFile(file: File): Promise<ParseResult> {
  ...
  rawRows.forEach((raw, index) => {
    ...
    for (const prefix of ['Presentacion1', 'Presentacion2', 'Presentacion3']) {
      ...
      if (!pName || pFactor <= 1 || pPrice <= 0) {
        errors.push({
          rowNumber,
          message: `${prefix}: debe tener nombre, unidades > 1 y precio > 0`,
        })
        continue
      }

      presentations.push({ name: pName, factor: pFactor, price: pPrice, barcode: pBarcode })
    }
